...
Enhanced version with daily evaluation (instead of monthly rebalancing)

Key changes from original:
- Daily evaluation: evaluate prediction every day, not just monthly
- Compare with 30-day forward returns for each day
- More data points for model assessment
"""
...
OUTPUT_DIR = "output_train_30d_daily"
...
def evaluate_daily(pred_df, top_k=TOP_K, bottom_k=BOTTOM_K):
    """
    NEW: Daily evaluation - evaluate every day instead of monthly rebalancing.
    This provides more data points and reduces noise from monthly sampling.

    For each day:
    - Select top_k stocks with highest predicted returns (long)
    - Select bottom_k stocks with lowest predicted returns (short)
    - Calculate spread = mean(long returns) - mean(short returns)
    """
    if pred_df.empty:
    ...
    for rebal_date in dates:
        ...
        # Long: top_k predicted returns
        # Short: bottom_k predicted returns
        ...
        # Hit ratio: how many stocks in top actually went up, and bottom went down
...
def evaluate_monthly(pred_df, top_k=TOP_K, bottom_k=BOTTOM_K):
    """
    Original monthly evaluation for comparison.
    """
    if pred_df.empty:
        return {"num_days": 0, "sharpe": np.nan, "hit_ratio": np.nan, "spread": np.nan, "daily_df": None}
    ...
    daily_df = pd.DataFrame(daily_results)
    daily_df["spread_after_cost"] = daily_df["spread"]
...
def main():
    ...
    log("=" * 60)
    log("JPX 30-Day Horizon - Daily Evaluation Mode")
    log("=" * 60)
    ...
    # ===== Daily Evaluation (NEW) =====
    log("\n" + "=" * 50)
    log("DAILY EVALUATION (Every trading day)")
    log("=" * 50)
    daily_metrics = evaluate_daily(pred)

    log(f"Number of trading days evaluated: {daily_metrics['num_days']}")
    ...
    # ===== Monthly Evaluation (Original for comparison) =====
    log("\n" + "=" * 50)
    log("MONTHLY EVALUATION (Original - for comparison)")
    log("=" * 50)
    monthly_metrics = evaluate_monthly(pred)

    log(f"Number of rebalancing months: {monthly_metrics['num_days']}")
    log(f"Total Monthly Spread: {monthly_metrics['spread']:.4%}")
    log(f"Sharpe Ratio: {monthly_metrics['sharpe']:.4f}")
    log(f"Hit Ratio: {monthly_metrics['hit_ratio']:.2%}")
    ...
    log(f"Spearman Correlation: {pred_metrics['spearman']:.4f}")
    log(f"Hit Ratio (direction): {pred_metrics['hit']:.2%}")

    # ===== Monthly breakdown for daily evaluation =====
    if daily_metrics["daily_df"] is not None and not daily_metrics["daily_df"].empty:
        daily_df = daily_metrics["daily_df"]
        log("\n" + "=" * 50)
        log("MONTHLY SPREAD BREAKDOWN (Daily Eval)")
        log("=" * 50)
    ...
    metrics = {
        **pred_metrics,
        "daily_num_days": daily_metrics["num_days"],
        "daily_sharpe": daily_metrics["sharpe"],
        "daily_avg_spread": daily_metrics["avg_spread"],
        "daily_spread": daily_metrics["spread"],
        "daily_hit_ratio": daily_metrics["hit_ratio"],
        "monthly_num_days": monthly_metrics["num_days"],
        "monthly_sharpe": monthly_metrics["sharpe"],
        "monthly_spread": monthly_metrics["spread"],
        "monthly_hit_ratio": monthly_metrics["hit_ratio"],
    }
    ...
    # Save daily results
    if daily_metrics["daily_df"] is not None:
        daily_path = os.path.join(OUTPUT_DIR, "daily_results.csv")
        daily_metrics["daily_df"].to_csv(daily_path, index=False)
