"""
LGBM Walkforward Prediction - 20d Locked Portfolio Experiment

This experiment tests LGBM with:
...
Key features:
- Forced 20-day lock period - no early rebalancing allowed
- Simple return accumulation (no compounding)
- Turnover calculated separately

Output:
- output_lgbm_20d_locked/predictions.csv
- output_lgbm_20d_locked/metrics.csv
"""
...
OUTPUT_DIR = "output_lgbm_20d_locked"
...
LOCK_DAYS = 20  # Forced 20-day lock period
...
def evaluate_portfolio_locked(pred_df):
    """
    Evaluate portfolio with 20-day locked portfolio.
    ...
    if pred_df.empty:
        return {
            ...
            "total_turnover": np.nan, "rebalance_count": np.nan,
        }

    tmp = pred_df[["Date", "SecuritiesCode", "pred", "y_true"]].copy()
    tmp = tmp.rename(columns={"y_true": "Target"})

    # Use 20-day locked portfolio
    _, daily_perf, m = base.construct_20d_locked_portfolio(
        tmp, pred_col="pred", target_col="Target",
        long_k=200, short_k=200, lock_days=LOCK_DAYS,
        trading_cost_rate=base.TRADING_COST_RATE, slippage_rate=base.SLIPPAGE_RATE,
    )

    # Additional turnover metrics
    total_turnover = float(daily_perf["turnover"].sum()) if not daily_perf.empty else np.nan
    rebalance_count = int(daily_perf["is_rebalance"].sum()) if not daily_perf.empty else 0
...
def plot_predictions(pred_df):
    ...
    # 3. Cumulative return (simple, not compounded)
    tmp = pred_df[["Date", "SecuritiesCode", "pred", "y_true"]].copy()
    tmp = tmp.rename(columns={"y_true": "Target"})
    _, daily_perf, _ = base.construct_20d_locked_portfolio(
        tmp, pred_col="pred", target_col="Target",
        long_k=200, short_k=200, lock_days=LOCK_DAYS,
        trading_cost_rate=base.TRADING_COST_RATE, slippage_rate=base.SLIPPAGE_RATE,
    )
    if not daily_perf.empty:
        ...
        ax3.set_title("Portfolio Cumulative Return (Simple)")
        ax3.set_xlabel("Date")
        ax3.set_ylabel("Cumulative Return")
        ax3.grid(True, alpha=0.3)

    # 4. Turnover by rebalance
    if not daily_perf.empty:
        rebalance_turnover = daily_perf[daily_perf["is_rebalance"] == 1]["turnover"]
        ax4 = axes[1, 1]
        ax4.bar(range(len(rebalance_turnover)), rebalance_turnover.values)
        ax4.set_title("Turnover at Each Rebalance (Every 20 days)")
        ax4.set_xlabel("Rebalance Index")
...
def main():
    ...
    log("=" * 60)
    log("LGBM Walkforward - 20d Locked Portfolio Experiment")
    log("=" * 60)
    log(f"Configuration:")
    log(f"  - Data: stock+all (requires options, trades, financials)")
    log(f"  - Horizon: {TARGET_HORIZON}d")
    log(f"  - Lock period: {LOCK_DAYS} days (forced hold)")
    log(f"  - Return: Simple accumulation (no compounding)")
    ...
    port = evaluate_portfolio_locked(pred)
    ...
    log("\n" + "=" * 40)
    log("PORTFOLIO METRICS (20d Locked)")
    log("=" * 40)
    log(f"Total Return (Simple): {port['portfolio_total_return']:.2%}")
    ...
    log(f"Total Turnover: {port['total_turnover']:.2f}")
    log(f"Rebalance Count: {port['rebalance_count']}")
